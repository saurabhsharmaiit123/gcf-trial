async function CreateLabsheetWithoutList(agent) {
    ...
    if (createColumnsContext) {
      ...
      const question = `
        Using the labsheet name ${labsheetName}, ${numberOfColumns} and ${columnType} and keep the column_name same as column_type an generate a json payload in the output using the following description:
        To create a labsheet you need to provide a name and a list of columns. 
        use this json for the labsheet: 
        {
            "apiKey": "api_key", // Replace with the actual API key
            "name": "${labsheetName}",
            "columns": []
        } 
